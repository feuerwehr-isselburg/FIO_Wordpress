...
function mkPluginLoadingIndic($which_div, show_hide_status) {
    var loading_indic = '<div class="mk-plugin-loading-holder"><div class="mk-btn-loading-spinner"></div></div>';
    var loading_txt = '<h4 class="mk-load-more-txt">Load More</h4>';
    if (show_hide_status) {
        $which_div.empty().html(loading_indic);
    } else {
        $which_div.empty().html(loading_txt);
    }
}
...
function mkApiPluginTemplateGenerator(data) {
    ...
    var template = '<div class="mk-plugin-tbl-row"><div class="mk-plugin-tbl-col-icon">' +
        '<div class="icon"></div></div><div class="mk-plugin-tbl-col-title-desc">' +
        '<div class="mk-plugin-name"><span class="mk-bold">' + data.name + '</span>' +
        '<span class="mk-plugin-subtitle">Version ' + data.version + '</span></div>' +
        '<div class="mk-plugin-desc">' + data.desc.substring(0, 85) + ' ...' + '</div></div>' +
...
function mkInstalledPluginTemplateGenerator(data) {
    ...
    var template = '<div class="mk-plugin-tbl-row"><div class="mk-plugin-tbl-col-icon">' +
        ...
        '</div><div class="mk-plugin-desc">' + data.desc.substring(0, 85) + ' ...' +
...
function mkActivatePlugin(plugin_slug) {
    ...
    jQuery.ajax({
        ...
        timeout: 60000,
        success: function(response) {
            console.log('Install Plugin - Request :', req_data, ' Response :', response);
            if (response.hasOwnProperty('status')) {
                if (response.status == true) {
