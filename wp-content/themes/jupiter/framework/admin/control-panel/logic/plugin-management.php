...
class mk_plugin_management {
	...
	public function __construct( $system_test_env = false, $ajax_mode = true ) {
		...
		if ( $ajax_mode == true ) {
			...
			add_action( 'wp_ajax_abb_installed_plugins', array( &$this, 'getListOfInstalledPlugin' ) );
		}
	}
	/**
	...
	public function abbRemovePlugin() {
		try {
			...
			if ( $plugin_head_file_name == false ) {
				throw new Exception( __( "Can't find plugin head file. {abb_remove}", 'mk_framework' ) );
			}
			...
			if ( $this->removePlugin( $this->getPluginFullPath(), $this->getPluginsDir() ) == false ) {
				throw new Exception( __( 'Cant delete directory', 'mk_framework' ) );
			}

			if ( $this->getActivePlugins( $this->getPluginHeadFileName() ) == false ) {
				$this->message( __( 'Successfull', 'mk_framework' ), true );
				return true;
			} else {
				throw new Exception( __( 'Failure in removing', 'mk_framework' ) );
			}
		} catch (Exception $e) {
	...
	public function abbUpdatePlugin() {
		try {
			...
			$this->deactivatePlugin( $this->getPluginName() );
			if ( $this->removePlugin( $this->getPluginFullPath(), $this->getPluginsDir() ) == false ) {
				throw new Exception( __( "Can't delete plugin directory", 'mk_framework' ) );
			}
			if ( $this->getActivePlugins( $this->getPluginHeadFileName() ) == false ) {
				return $this->install();
			} else {
	...
	public function abbLazyPluginLoad() {
		...
		if ( is_null( $from ) || is_null( $count ) ) {
			$this->message( __( 'System problem , please contact support', 'mk_framework' ), false );
			return false;
		}
			...
			if ( is_array( $list_of_plugins ) && count( $list_of_plugins ) > 0 ) {
				foreach ( $list_of_plugins as $key => $plugin_info ) {
					if ( $this->getActivePlugins( $plugin_info['slug'] ) ) {
						// Remove already installed plugin from list.
	...
	public function getListOfInstalledPlugin() {
		try {
			$list_of_plugins = $this->getPluginListFromApi();
			if ( is_array( $list_of_plugins ) && count( $list_of_plugins ) > 0 ) {
				foreach ( $list_of_plugins as $key => $plugin_info ) {
				...
				$list_of_plugins = array_slice( $list_of_plugins, $from, $count );
				$this->message( __( 'successfull', 'mk_framework' ), true, $list_of_plugins );
				return true;
			} else {
				$this->message( 'Plugin list is empty', false );
				throw new Exception( __( 'Plugin list is empty', 'mk_framework' ) );
			}
		} catch (Exception $e) {
			return false;
		}// End try().
	...
	public function installBatch() {
		try {
			...
			if ( empty( $plugins_list ) || is_array( $plugins_list ) == false || count( $plugins_list ) == 0 ) {
				throw new Exception( __( 'Plugin list is not an array , use install method instead', 'mk_framework' ) );
			}
			...
			$message_need_to_replace = __( "{%s} plugins installed successfully'", 'mk_framework' );
			$this->message( sprintf( $message_need_to_replace , count( $plugins_list ) ) );
			return true;
	...
	public function install() {
		try {
			...
			if ( $this->getPluginName() == '' ) {
				throw new Exception( __( 'Choose a plugin first.', 'mk_framework' ) );
			}
			...
			if ( ($head_file_response = $this->findPluginHeadFileName( $this->getPluginName() )) !== false ) {
				if ( $this->getActivePlugins( $head_file_response ) == false ) {
					...
					$this->message( __( 'Plugin successfully added and activated.', 'mk_framework' ), true );
					return true;
				} else {
					$this->message( __( 'Plugin successfully added and activated.', 'mk_framework' ), true );
					return true;
			...
			if ( $this->isWritable( $this->getPluginsDir() ) == false ) {
				throw new Exception( __( 'Plugin directory is not writable.', 'mk_framework' ) );
			}
			...
			if ( filter_var( $api_response['source'], FILTER_VALIDATE_URL ) === false ) {
				throw new Exception( __( 'Plugin source not found', 'mk_framework' ) );
			}
			...
			if ( $found_file === false ) {
				$need_to_be_replace = __( "Can't find %s plugin head file name.", 'mk_framework' );
				throw new Exception( sprintf( $need_to_be_replace , $this->getPluginName() ) );
			}
	...
	public function updatePlugin() {
		try {
			// Validate if plugin name is setted
			if ( $this->getPluginName() == '' ) {
				throw new Exception( __( 'Plguin information data format is not right.', 'mk_framework' ) );
			}

			$found_file = $this->findPluginHeadFileName( $this->getPluginName() );
			if ( $found_file == false ) {
				throw new Exception( __( "Can't find plugin head file name.", 'mk_framework' ) );
			}
			...
			$this->message( __( 'Update completed', 'mk_framework' ), true );
			return true;
		} catch (Exception $e) {
			$this->message( $e->getMessage(), false );
			return false;
	...
	public function unzipPlugin( $zip_path ) {
		...
		if ( ! $wp_filesystem ) {
			WP_Filesystem();
			if ( ! $wp_filesystem ) {
				throw new Exception( __( 'unzipPlugin , System Error 100x001', 'mk_framework' ) );
				return false;
			}
		}
	...
	public function removePlugin( $plugin_full_path, $plugins_dir ) {
		...
		try {
			if ( $this->isWritable( $plugins_dir ) == false ) {
				throw new Exception( __( 'Plugin parent directory is not writable {remove Plugin}.', 'mk_framework' ) );
			}
			...
			if ( strlen( str_replace( $plugins_dir, '', $plugin_full_path ) ) > 2 ) {
				if ( $this->isWritable( $plugin_base_directory ) == false ) {
					throw new Exception( __( 'Plugin directory is not writable {remove Plugin}.', 'mk_framework' ) );
				}

				if ( $this->deleteDirectory( $plugin_base_directory ) ) {
					return true;
				} else {
					throw new Exception( __( "Can't remove directory of plugin - Directory", 'mk_framework' ) );
				}
			} else {
				if ( $this->deleteDirectory( $plugin_full_path ) ) {
					return true;
				} else {
					throw new Exception( __( "Can't remove directory of plugin - File", 'mk_framework' ) );
				}
			}
		} catch (Exception $e) {
			$this->message( $e->getMessage(), false );
	...
	public function uploadPluginFromURL( $url, $name ) {
		if ( $this->checkRemoteFileExistence( $url ) == false ) {
			throw new Exception( __( "Can't download plugin source file.", 'mk_framework' ) );
			return false;
		}
		...
		if ( function_exists( 'curl_version' ) ) {
			...
			if ( $fp == false ) {
				throw new Exception( __( "Can't open destination file", 'mk_framework' ) );
				return false;
			}
			$ch = curl_init( $url );
			...
			if ( $response == false ) {
				throw new Exception( __( "Can't download file using put contents , Please contact webmaster.", 'mk_framework' ) );
				return false;
			} else {
				return true;
			}
	...
	public function getPluginListFromApi( $plugin_name = false ) {
		...
		$api_response = [
			[
				'name'    => 'Hello Dolly',
				'img_url' => 'https://dummyimage.com/200x100/000/ffffff&text=' . 'Hello Dolly',
				'slug'    => 'hello-dolly',
				'source'  => 'wp-repo',
				'desc'    => 'wp-repo',
				'version' => 4.0,
			...
			[
				...
				'img_url' => 'https://dummyimage.com/200x100/000/ffffff&text=' . 'Artbees Themes Captcha',
			...
			[
				'name'    => 'WP Super Cache',
				'img_url' => 'https://dummyimage.com/200x100/000/ffffff&text=' . 'WP Super Cache',
				'slug'    => 'wp-super-cache',
				'desc'    => 'wp-repo',
				'source'  => 'wp-repo',
				'version' => 'wp-repo',
			...
			[
				'name'    => 'Akismet',
				'img_url' => 'https://dummyimage.com/200x100/000/ffffff&text=' . 'Akismet',
				'slug'    => 'akismet',
				'desc'    => 'wp-repo',
				'source'  => 'wp-repo',
				'version' => 'wp-repo',
		...
		if ( is_array( $result ) && count( $result ) > 0 ) {
			foreach ( $result as $key => $value ) {
				...
				if ( is_array( $fetch_data ) && count( $fetch_data ) > 0 ) {
					$response = $this->getPluginInfoFromWPRepo( $value['slug'], $fetch_data );
					if ( $response != false ) {
			...
			throw new Exception( __( 'The plugin you are looking for is not exist.', 'mk_framework' ) );
			return false;
		}
	}
	/**
	...
	public function getPluginInfoFromWPRepo( $slug, $info_array = array() ) {
		...
		if ( ! isset( $api[ $slug ] ) ) {
			...
			$response   = plugins_api( 'plugin_information', array( 'slug' => $slug, 'fields' => array( 'sections' => false, 'short_description' => true ) ) );
			$api[ $slug ] = false;
