...
class mk_template_managememnt {
	...
	public function abbInstallTemplateProcedure() {
		...
		if ( is_null( $template_name ) || is_null( $type ) ) {
			$this->message( __( 'System problem at installing , please contact support', 'mk_framework' ), false );
	...
	public function reinitializeData( $template_name ) {
		try {
			if ( empty( $template_name ) ) {
				throw new Exception( __( 'Choose template first', 'mk_framework' ) );
			}
			...
			if (
				...
				throw new Exception( __( 'Template assets are not completely exist - p1, Contact support.', 'mk_framework' ) );
			} else {
				return true;
			}
		} catch (Exception $e) {
	...
	public function abbTemplatelazyLoadFromApi() {
		try {
			...
			if ( is_null( $from ) || is_null( $count ) ) {
				throw new Exception( __( 'System problem , please contact support', 'mk_framework' ), 1001 );
				return false;
			}
			...
			if ( is_array( $list_of_templates ) ) {
				$this->message( __( 'Successfull', 'mk_framework' ), true, $list_of_templates );
				return true;
			} else {
				throw new Exception( __( 'Template list is not what we expected', 'mk_framework' ) );
			}
		} catch (Exception $e) {
			$this->message( $e->getMessage(), false );
			return false;
	...
	public function uploadTemplateToServer( $template_name ) {
		try {
			...
			if ( empty( $getTemplateName ) ) {
				throw new Exception( __( 'Choose one template first', 'mk_framework' ) );
			}
			...
			if ( filter_var( $url, FILTER_VALIDATE_URL ) === false ) {
				throw new Exception( __( 'Template source URL is not validate', 'mk_framework' ) );
			}
			$this->uploadFromURL( $this->getBasePath(), $this->getTemplateRemoteAddress() , $template_file_name );
			$this->message( __( 'Uploaded to server', 'mk_framework' ), true );
			return true;
		} catch (Exception $e) {
			$this->message( $e->getMessage(), false );
			return false;
	...
	public function unzipTemplateInServer( $template_name ) {
		try {
			...
			if ( $this->deleteDirectory( $this->getBasePath() . $this->getTemplateFileName() ) == false ) {
				throw new Exception( __( 'Cannot delete template zip file', 'mk_framework' ) );
			}

			$this->message( __( 'Compeleted', 'mk_framework' ), true );
	...
	public function validateTemplateFiles( $template_name ) {
		try {
			...
			if (
				...
				throw new Exception( __( 'Template assets are not completely exist - p2, Contact support.', 'mk_framework' ) );
			} else {
				$this->message( __( 'Compeleted', 'mk_framework' ), true );
	...
	public function installRequiredPlugins( $template_name ) {
		...
		try {
			...
			if ( is_array( $plugins['required_plugins'] ) == false or count( $plugins['required_plugins'] ) <= 0 ) {
				throw new Exception( __( 'Plugin set have wrong structure', 'mk_framework' ) );
			}
	...
	public function importThemeContent( $template_name ) {
		try {
			...
			ob_end_clean();
			$this->message( __( 'Template contents were imported.', 'mk_framework' ), true );
	...
	public function importMenuLocations( $template_name ) {
		try {
			...
			$this->message( __( 'Navigation locations is configured.', 'mk_framework' ), true );
	...
	public function importThemeOptions( $template_name ) {
		try {
			...
			if ( empty( $data ) == false ) {
				...
				$this->message( __( 'Theme options are imported.', 'mk_framework' ), true );

				return true;
			} else {
				throw new Exception( __( 'Template options is empty', 'mk_framework' ) );
	...
	public function importThemeWidgets( $template_name ) {
		...
		try {
			...
			$this->message( __( 'Widgets are imported.', 'mk_framework' ), true );
	...
	public function finilizeImporting( $template_name ) {
		...
		try {
			...
			if ( $delete_response = false ) {
				throw new Exception( __( "Can't remove source files", 'mk_framework' ) );
	...
	private function importWidgetData( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			throw new Exception( __( 'Widget data could not be read. Please try a different file.', 'mk_framework' ) );
		}
		$results = array();
				...
				if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							$fail = true;
							$widget_message_type = __( 'warning', 'mk_framework' );
	...
	public function uploadFromURL( $upload_dir, $url, $name ) {
		if ( $this->checkRemoteFileExistence( $url ) == false ) {
			throw new Exception( __( "Can't download template source file.", 'mk_framework' ) );
			return false;
		}
	...
	public function unzipFile( $zip_path, $dest_path ) {
		...
		if ( ! $wp_filesystem ) {
			WP_Filesystem();
			if ( ! $wp_filesystem ) {
				throw new Exception( __( 'Unzip Template , System Error 100x001', 'mk_framework' ) );
	...
	public function getTemplateCategoryListFromApi() {
		try {
			...
			$this->message( __( 'Successfull', 'mk_framework' ), true, $response->body->data );
			return true;
		} catch (Exception $e) {
			$this->message( $e->getMessage(), false );
			return false;
	...
	public function getAssetsAddress( $which_one, $template_name ) {
		switch ( $which_one ) {
			...
			break;
			default:
			throw new Exception( __( 'File name you are looking for is not introduced.', 'mk_framework' ) );
	...
	public function importLayerSliderContent( $content_path ) {
		...
		if ( $ls_head_file == false ) {
			throw new Exception( __( 'LayerSlider is not installed , install it first', 'mk_framework' ) );

			return false;
		}
		if ( $plugin->getActivePlugins( $ls_head_file ) == false ) {
			throw new Exception( __( 'LayerSlider is installed but not activated , activate it first', 'mk_framework' ) );
			return false;
		}
