...
class Mk_Addon_Management {
	...
	public function abbAddonLazyLoad() {
		...
		if ( is_null( $from ) || is_null( $count ) ) {
			$this->message( __( 'System problem , please call support' , 'mk_framework' ), false );
			return false;
		}
	...
	public function abbInstalledAddons() {
		...
		if ( count( $installed_addons ) < 1 ) {
			$this->message( __( 'Successfull' , 'mk_framework' ), true, array() );
			return true;
		}
		...
		$this->message( __( 'Successfull' , 'mk_framework' ), true, $installed_addons );
		return true;
	}
	public function abbActivateAddon() {
		try {
			...
			if ( is_null( $addon_slug ) ) {
				throw new Exception( __( 'System problem at installing , please call support' , 'mk_framework' ) );
				return false;
			}
	...
	public function update() {
		try {
			...
			if ( count( $api_response ) < 1 ) {
				throw new Exception( __( 'Add-on you are looking for is not exist in API side' , 'mk_framework' ) );
			}
			...
			if ( $version_cmp === false ) {
				throw new Exception( __( 'You have latest version of this add-on.' , 'mk_framework' ) );
			}
	...
	public function validateAddonStructure( $addon_path ) {
		...
		foreach ( $must_exist_files as $key => $value ) {
			if ( file_exists( $addon_path . $value ) === false ) {
				throw new ValidateAddonException( __( 'Add-On structure have problem , Please download it again' , 'mk_framework' ) );
				return false;
			}
		}
		return true;
